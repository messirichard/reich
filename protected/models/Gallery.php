...
class Gallery extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('city, harga, color, product_id', 'safe'),
  ...
  //   	$pages->applyLimit($criteria);
	...
	public function getGallerySpotlight_home()
	{
		$command = Yii::app()->db->createCommand("SELECT t1.nama as nama_spot, t1.product_id as product_id, t2.gallery_id as gallery_id FROM home_product_spot as t1  INNER JOIN prd_gallery_highlight as t2 ON t1.product_id = t2.product_id")
					->queryAll();
		$data = array();
		foreach ($command as $key => $value) {
			$data[$key] = [
						'nama'=>$value['nama_spot'],
